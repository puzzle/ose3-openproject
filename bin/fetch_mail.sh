...
if [ -n "${REDMINE_MAIL_INTEGRATION}" ]; then
  ...
  IMAP_SETTINGS="${IMAP_SETTINGS} no_permission_check=1 unknown_user=accept allow_override=project,tracker,type,status,priority,category,assigned_to,fixed_version,start_date,due_date,estimated_hours,done_ratio"
